...
# if target_column != previous_target:
#     st.session_state.top_feature = None
...
if st.button("Setup ML Environment & Compare Models"):
    with st.spinner("Preparing ML environment..."):
        if problem_type == 'regression':
            reg.setup(data=df, target=target_column, session_id=123, verbose=False)
        else:
            clf.setup(data=df, target=target_column, session_id=123, verbose=False)
